...
export default function Page() {
  ...
  >();
  const [
    provinceLegislativeDistrictCount,
    setProvinceLegislativeDistrictCount,
  ] = useState<number | undefined>();

  const [councilorDistrict, setCouncilorDistrict] = useState<
    number | undefined
  >();
  const [councilorDistrictCount, setCouncilorDistrictCount] = useState<
    number | undefined
  >();
  ...
  const [legislativeDistrict, setLegislativeDistrict] = useState<
    number | undefined
  >();
  const [legislativeDistrictCount, setLegislativeDistrictCount] =
    useState<number>(0);
  ...
  const [hasLocalRepresentatives, setHasLocalRepresentatives] = useState<
    boolean | undefined
  >();
  ...
  >();
  const [selectedViceGovernor, setSelectedViceGovernor] = useState<
    LocalCandidate | undefined
  >();
  const [selectedMayor, setSelectedMayor] = useState<
    LocalCandidate | undefined
  >();
  const [selectedViceMayor, setSelectedViceMayor] = useState<
    LocalCandidate | undefined
  >();
  const [selectedRepresentative, setSelectedRepresentative] = useState<
    LocalCandidate | undefined
  >();
  const [selectedProvincialBoard, setSelectedProvincialBoard] = useState<
    LocalCandidate[] | undefined
  >();
  ...
  async function onSubmit(values: z.infer<typeof locationSchema>) {
    ...
    localStorage.setItem(
      ...
      hasLocalRepresentatives ? "true" : "false",
    );
    ...
    // }
    localStorage.setItem(
      "legislativeDistrictCount",
      legislativeDistrictCount?.toString() ?? 0,
    );
  ...
  async function onProvinceChange(provinceName: string) {
    ...
    setLegislativeDistrict(undefined);
    setProvinceLegislativeDistrictCount(
      provinces[0].total_legislative_district,
    );
  ...
  async function onLGUChange(lguName: string) {
    ...
    if (region !== "NCR") {
      const res = await fetch(
        `/api/location/lgus/has-provincial-rep?l=${lguName}&p=${newProvince}`,
      );
  ...
  async function retrieveCandidates(
    ...
    console.log(pd);
    const candidatesReq = await fetch(
      `/api/local-candidates?r=${r}&l=${l}&ld=${ld}&cd=${cd}${pd !== undefined ? `&pd=${pd}` : ""}${p !== undefined ? `&p=${p}` : ""}`,
    );
  ...
  const removeCouncilor = (councilor: LocalCandidate) => {
    setSelectedCouncilors(
      selectedCouncilors.filter((c) => c.ballot_name !== councilor.ballot_name),
    );
  ...
  const selectProvincialBoardMember = (
    provincialBoardMember: LocalCandidate,
  ) => {
    ...
    if (!selectedProvincialBoard)
      setSelectedProvincialBoard([provincialBoardMember]);
    else
      setSelectedProvincialBoard([
        ...(selectedProvincialBoard ?? []),
        provincialBoardMember,
      ]);

    db.localCandidates.add(provincialBoardMember, provincialBoardMember.id);
  };
  const removeProvincialBoardMember = (
    provincialBoardMember: LocalCandidate,
  ) => {
    setSelectedProvincialBoard(
      selectedProvincialBoard?.filter(
        (pb) => pb.ballot_name !== provincialBoardMember.ballot_name,
      ),
    );
  ...
  const retrievePreviouslySetCandidates = async () => {
    const mayor = await db.localCandidates
      .where("position")
      .equals("MAYOR")
      .first();
    if (mayor) setSelectedMayor(mayor);
    const viceMayor = await db.localCandidates
      .where("position")
      .equals("VICE-MAYOR")
      .first();
    if (viceMayor) setSelectedViceMayor(viceMayor);
    const governor = await db.localCandidates
      .where("position")
      .equals("GOVERNOR")
      .first();
    if (governor) setSelectedGovernor(governor);
    const viceGovernor = await db.localCandidates
      .where("position")
      .equals("VICE-GOVERNOR")
      .first();
    if (viceGovernor) setSelectedViceGovernor(viceGovernor);
    const representative = await db.localCandidates
      .where("position")
      .equals("REPRESENTATIVE")
      .first();
    if (representative) setSelectedRepresentative(representative);
    const councilors = await db.localCandidates
      .where("position")
      .equals("COUNCILOR");
    setSelectedCouncilors(await councilors.toArray());
    const provincialBoardMembers = await db.localCandidates
      .where("position")
      .equals("PROVINCIAL_COUNCIL")
      .toArray();
                      ...
                      {governors.map((g) => (
                        <CandidateRow
                          ...
                          isSelected={
                            ...
                            selectedGovernor?.ballot_name === g.ballot_name
                          }
                        />
                      ))}
                    </div>
                    ...
                    <div className="flex flex-col gap-2 mt-3">
                      {viceGovernors.map((vg) => (
                        <CandidateRow
                          ...
                          details={vg}
                          isSelected={
                            selectedViceGovernor != undefined &&
                            selectedViceGovernor?.ballot_name === vg.ballot_name
                          }
                        />
                      ))}
                    </div>
                  ...
                  {mayors.map((m) => (
                    <CandidateRow
                      ...
                      isSelected={
                        ...
                        selectedMayor?.ballot_name === m.ballot_name
                      }
                    />
                  ))}
                </div>
                ...
                <div className="flex flex-col gap-2 mt-3">
                  {viceMayors.map((vm) => (
                    <CandidateRow
                      ...
                      details={vm}
                      isSelected={
                        selectedViceMayor != undefined &&
                        selectedViceMayor?.ballot_name === vm.ballot_name
                      }
                    />
                  ))}
                </div>
                  ...
                  {representatives.map((rm) => (
                    <CandidateRow
                      ...
                      isSelected={
                        ...
                        selectedRepresentative?.ballot_name === rm.ballot_name
                      }
                    />
                  ))}
                </div>
                    ...
                    {provincialBoard &&
                      provincialBoard.map((vm) => (
                        <CandidateRow
                          ...
                          details={vm}
                          isSelected={
                            selectedProvincialBoard != undefined &&
                            selectedProvincialBoard.filter(
                              (pb) => pb.ballot_name === vm.ballot_name,
                            ).length !== 0
                          }
                        />
                      ))}
                ...
                <div className="flex flex-col gap-2 mt-3">
                  {councilors.map((c) => (
                    <CandidateRow
                      ...
                      details={c}
                      isSelected={
                        selectedCouncilors.filter(
                          (sc) => sc.ballot_name === c.ballot_name,
                        ).length !== 0
                      }
                    />
                  ))}
            ...
            <DialogContent className="w-11/12">
              ...
              <Form {...locationForm}>
                <form onSubmit={locationForm.handleSubmit(onSubmit)}>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select
                          ...
                          defaultValue={province ?? undefined}
                          disabled={!regionSelected || isNcrPicked}
                        >
                          <FormControl>
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select
                          ...
                          defaultValue={lgu ?? undefined}
                          disabled={!provinceSelected && !isNcrPicked}
                        >
                          <FormControl>
                            <SelectTrigger className="w-full ">
                  ...
                  <FormField
                    ...
                    render={({ field }) => (
                      <FormItem>
                        <FormLabel>
                          {hasLocalRepresentatives
                            ? "Legislative District (Local)"
                            : "Legislative District " + `(Provincial)`}
                        </FormLabel>
                        <Select
                          onValueChange={(val) => {
                          ...
                          defaultValue={
                            lgu !== undefined &&
                            legislativeDistrict !== undefined
                              ? legislativeDistrict.toString()
                              : undefined
                          }
                          disabled={!lguSelected}
                        >
                          <FormControl>
                            <SelectTrigger className="w-full ">
                              <SelectValue placeholder="Select City or Municipality" />
                          ...
                          <SelectContent>
                            {!newLgu ? (
                              ...
                              Array.from(
                                Array(legislativeDistrictCount).keys(),
                              ).map((num) => {
                                console.log(num + 1);
                              ...
                              Array.from(
                                Array(provinceLegislativeDistrictCount).keys(),
                              ).map((num) => {
                                const treatedNum = num + 1;
                                return (
                                  <SelectItem
                    ...
                    render={({ field }) => (
                      <FormItem>
                        ...
                        <Select
                          ...
                          defaultValue={
                            lgu !== undefined &&
                            provincialDistrict !== undefined
                              ? provincialDistrict.toString()
                              : undefined
                          }
                          disabled={!lguSelected || isNcrPicked}
                        >
                          <FormControl>
                            <SelectTrigger className="w-full ">
                              <SelectValue placeholder="Select City or Municipality" />
                          ...
                          <SelectContent>
                            {!newLgu ? (
                              ...
                              Array.from(
                                Array(provinceDistrictCount).keys(),
                              ).map((num) => {
                                const treatedNum = num + 1;
                                return (
                                  <SelectItem
                        ...
                        >
                          ...
                          <SelectContent>
                            {!newLgu ? (
                              ...
                              Array.from(
                                Array(councilorDistrictCount).keys(),
                              ).map((num) => {
                                const treatedNum = num + 1;
                                return (
                                  <SelectItem
                  ...
                  <Button
                    type="submit"
                    className="mt-4 w-full !bg-yellow-500 !rounded-xl"
                  >
                    Select Location
                  </Button>
                </form>
